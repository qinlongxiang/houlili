...
#error Unknown Compiler
    ...
    {
        vulkan_util::createImage(m_physical_device,
                                m_device,
        ...
        ((VulkanImageView*)m_depth_image_view)->setResource(
            vulkan_util::createImageView(m_device, ((VulkanImage*)m_depth_image)->getResource(), (VkFormat)m_depth_image_format, VK_IMAGE_ASPECT_DEPTH_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1));
    }
    ...
    RHISampler* VulkanRHI::getOrCreateDefaultSampler(RHIDefaultSamplerType type)
    {
        switch (type)
        {
        case HouLili::Default_Sampler_Linear:
            if (m_linear_sampler == nullptr)
            {
                m_linear_sampler = new VulkanSampler();
                ((VulkanSampler*)m_linear_sampler)->setResource(vulkan_util::getOrCreateLinearSampler(m_physical_device, m_device));
            }
            return m_linear_sampler;
            break;

        case HouLili::Default_Sampler_Nearest:
            if (m_nearest_sampler == nullptr)
            {
                m_nearest_sampler = new VulkanSampler();
                ((VulkanSampler*)m_nearest_sampler)->setResource(vulkan_util::getOrCreateNearestSampler(m_physical_device, m_device));
            }
    ...
    RHISampler* VulkanRHI::getOrCreateMipmapSampler(uint32_t width, uint32_t height)
    {
        ...
        else
        {
            ...
            VkSampler vk_sampler = vulkan_util::getOrCreateMipmapSampler(m_physical_device, m_device, width, height);
    ...
    RHIShader* VulkanRHI::createShaderModule(const std::vector<unsigned char>& shader_code)
    {
        ...
        VkShaderModule vk_shader =  vulkan_util::createShaderModule(m_device, shader_code);
    ...
    void VulkanRHI::createBuffer(RHIDeviceSize size, RHIBufferUsageFlags usage, RHIMemoryPropertyFlags properties, RHIBuffer* & buffer, RHIDeviceMemory* & buffer_memory)
    {
        ...
        vulkan_util::createBuffer(m_physical_device, m_device, size, usage, properties, vk_buffer, vk_device_memory);
    ...
    void VulkanRHI::createBufferAndInitialize(RHIBufferUsageFlags usage, RHIMemoryPropertyFlags properties, RHIBuffer*& buffer, RHIDeviceMemory*& buffer_memory, RHIDeviceSize size, void* data, int datasize)
    {
        ...
        vulkan_util::createBufferAndInitialize(m_device, m_physical_device, usage, properties, &vk_buffer, &vk_device_memory, size, data, datasize);
    ...
    void VulkanRHI::copyBuffer(RHIBuffer* srcBuffer, RHIBuffer* dstBuffer, RHIDeviceSize srcOffset, RHIDeviceSize dstOffset, RHIDeviceSize size)
    {
        ...
        vulkan_util::copyBuffer(this, vk_src_buffer, vk_dst_buffer, srcOffset, dstOffset, size);
    }
        ...
        RHIImageView* &image_view)
    {
        ...
        vk_image_view = vulkan_util::createImageView(m_device, vk_image, (VkFormat)format, image_aspect_flags, (VkImageViewType)view_type, layout_count, miplevels);
    ...
    void VulkanRHI::createGlobalImage(RHIImage* &image, RHIImageView* &image_view, VmaAllocation& image_allocation, uint32_t texture_image_width, uint32_t texture_image_height, void* texture_image_pixels, RHIFormat texture_image_format, uint32_t miplevels)
    {
        ...
        vulkan_util::createGlobalImage(this, vk_image, vk_image_view,image_allocation,texture_image_width,texture_image_height,texture_image_pixels,texture_image_format,miplevels);
    ...
    void VulkanRHI::createCubeMap(RHIImage* &image, RHIImageView* &image_view, VmaAllocation& image_allocation, uint32_t texture_image_width, uint32_t texture_image_height, std::array<void*, 6> texture_image_pixels, RHIFormat texture_image_format, uint32_t miplevels)
    {
        ...
        vulkan_util::createCubeMap(this, vk_image, vk_image_view, image_allocation, texture_image_width, texture_image_height, texture_image_pixels, texture_image_format, miplevels);
    ...
    void VulkanRHI::createSwapchainImageViews()
    {
        ...
        {
            VkImageView vk_image_view;
            vk_image_view = vulkan_util::createImageView(m_device,
    ...
    void VulkanRHI::destroyDefaultSampler(RHIDefaultSamplerType type)
    {
        ...
        {
        case HouLili::Default_Sampler_Linear:
            vulkan_util::destroyLinearSampler(m_device);
            delete(m_linear_sampler);
            break;
        case HouLili::Default_Sampler_Nearest:
            vulkan_util::destroyNearestSampler(m_device);
    ...
    {
        vulkan_util::destroyMipmappedSampler(m_device);
    ...
    HouLili::QueueFamilyIndices VulkanRHI::findQueueFamilies(VkPhysicalDevice physicalm_device) // for device and surface
    {
    ...
    HouLili::SwapChainSupportDetails VulkanRHI::querySwapChainSupport(VkPhysicalDevice physicalm_device)
    {
